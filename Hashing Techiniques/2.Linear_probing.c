...
int probe (int H[] , int key)               //Linear Probe
{
    int index = hash(key);
    int i = 0;
    
    while(H[(index + i) % SIZE] != 0)       //Hash Function Used = (( h'(x) = (h(x) + f(i)) % SIZE ))   ,where, i = 0 , 1 , 2 , 3 . . . . . . .
        i++;
...
int Search(int H[], int key)
{
    ...
    while(H[(index + i) % SIZE ] != key)
    {
        ...
        if((index + i) % SIZE == 0 )                    //this means all indexes have value therefore 
...
void Delete(int H[], int key)
{
    ...
    H[(index + i) % SIZE] = 0;                              //deleting memory by making it Zero.
